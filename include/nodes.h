...
/*
If you want each node to store multiple types of data (e.g., color, speed, size, position, heading,
etc.), you should define a struct or class that contains all the fields you need, and then use that
struct/class as the template parameter for your Node and LinkedList. For example: struct ObjectData
{ std::string color; double speed; double size; double position; double heading; Add more fields as
needed
};
 ...
 */
template <typename T> SingleNode<T>::SingleNode(const T& value) : data(value), next(nullptr) {}
 ...
 * Did two separate classes for single and double nodes to keep the code clean and maintainable and
 * reduce memory usage This class inherits from SingleNode and adds a pointer to the previous node.
 */
 ...
 * Description: Initializes a double node with the given value, sets the next pointer to nullptr,
 * and the previous pointer to nullptr. Parameters: value - The value to be stored in the node.
 * Returns: void - No return value.
 */
template <typename T>
