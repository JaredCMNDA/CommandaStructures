 ...
 *
 * Extra:
 * Overwrite only mode: new data is always accepted (push() never fails), the tail moves forward as
 * normal and when full, head also moves forward to discard the oldest item silently
 */
...
template <typename T> class RingBuffer
{
  public:
    ...
    [[nodiscard]] bool isOverwriteOnly() const
    {
        return overwriteOnly;
    }; // Checks if the buffer is in overwrite-only mode
    [[nodiscard]] size_t capacity() const
    {
        return maxCapacity;
    };                               // Returns the maximum number of elements the buffer can hold
    void resize(size_t newCapacity); // Resizes the buffer to a new capacity, preserving existing
 ...
 *             overwrite - A flag indicating if the buffer should overwrite the oldest element when
 * full (default is false). Returns: void - No return value.
 */
template <typename T>
 ...
 */
template <typename T>
RingBuffer<T>::~RingBuffer() = default; // Use the default destructor (no need for custom cleanup
                                        // since LinkedList handles its own memory)

/*
 * Name: RingBuffer.push
 * Description: Adds a new element to the buffer, overwriting the oldest element if the buffer is
 * full. Parameters: value - The value to be added to the buffer. Returns: void - No return value.
 */
template <typename T> void RingBuffer<T>::push(const T& value)
{
    if (isFull())
    {
        if (overwriteOnly)
 ...
 */
template <typename T> T& RingBuffer<T>::back() const
{
...
{
    return list.getSize() >= maxCapacity; // Check if the size of the linked list is equal to or
 ...
 * Description: Resizes the buffer to a new capacity, preserving existing elements if possible.
 * Removes oldest elements if the new capacity is smaller than the current size. Parameters:
 * newCapacity - The new maximum number of elements the buffer can hold. Returns: void - No return
 * value.
 */
template <typename T> void RingBuffer<T>::resize(size_t newCapacity)
{
