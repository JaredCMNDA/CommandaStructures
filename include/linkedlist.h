...
class LinkedList
{
  public:
    LinkedList();
    ~LinkedList();
    void insert(const T& value, int spot = TAIL); // insert a new node with the given value to the
                                                  // end of the list (spot is optional)
 ...
 */
template <typename T> LinkedList<T>::LinkedList() : size(0), head(nullptr), tail(nullptr) {}
 ...
 */
template <typename T> void LinkedList<T>::insert(const T& value, int spot)
{
 ...
 * Returns: void - No return value.
 */
template <typename T> template <typename Func> void LinkedList<T>::display(Func func) const
{
 ...
 */
template <typename T> SingleNode<T>* LinkedList<T>::findNode(const T& value) const
{
    ...
    while (current)
    {
        ...
        { // If the current node's data matches the value
            return const_cast<SingleNode<T>*>(
                current); // Return a non-const pointer to the found node
        }
 ...
 * Returns: void - No return value.
 */
template <typename T> void LinkedList<T>::removeNode(SingleNode<T>* node)
{
    if (!node || !head)
