...
void runRingBufferTest()
{
    ...
    /* Sample Use Case:
     * You want to maintain a fixed-size buffer for the last 10 temperature readings for temperature
     * sensitivity
     */
    ...
    RingBuffer<TemperatureReading> tempBuffer(
        10, true); // Create a ring buffer with a capacity of 10 and overwrite mode enabled
    ...
    for (int i = 0; i < rand() % 15 + 6; ++i)
    {
        ...
        double temperature =
            20.0 + (rand() % 100) / 10.0; // Random temperature between 20.0 and 30.0
        try
        {
    ...
    std::cout << "Is buffer empty after clearing? " << (tempBuffer.isEmpty() ? "Yes" : "No")
              << std::endl;
}